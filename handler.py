...
async def upload_output_file(
    s3_manager: S3Manager, local_path: str, filename: str
) -> str:
    """
...
def generate_audio(
    model: SparkTTS, params: Dict[str, Any], prompt_speech_path: Optional[str] = None
) -> torch.Tensor:
    """
...
async def handler(job):
    ...
    try:
        ...
        if params["prompt_speech_url"]:
            prompt_speech_path = await download_voice_file(
                s3_manager, params["prompt_speech_url"]
            )
        ...
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=f".{params['output_format']}"
        ) as tmp_file:
            temp_audio_path = tmp_file.name
        temp_files.append(temp_audio_path)

        save_audio_file(
            wav, temp_audio_path, model.sample_rate, params["output_format"]
        )

        # Upload to S3
        audio_url = await upload_output_file(
            s3_manager, temp_audio_path, output_filename
        )
        ...
        if params["generate_subtitles"] and prompt_speech_path:
            try:
                ...
                with tempfile.NamedTemporaryFile(
                    mode="w", delete=False, suffix=".ass"
                ) as ass_file:
                ...
                # Upload ASS file
                ass_url = await upload_output_file(
