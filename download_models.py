...
def download_huggingface_model(repo_url: str, local_path: str) -> bool:
    ...
    try:
        ...
        if check_git_lfs():
            ...
            lfs_result = subprocess.run(lfs_cmd, cwd=local_path, 
                                      capture_output=True, text=True)
    ...
    for model_name, config in MODELS_CONFIG.items():
        ...
        if os.path.exists(local_path):
            ...
            if verify_model_files(model_name, local_path, config['required_files']):
                ...
                logger.warning(f"Model {model_name} exists but is incomplete, re-downloading...")
...
def show_summary():
    ...
    try:
        ...
        if os.path.exists(SPARK_TTS_DIR):
            ...
            logger.info(f"Total Spark-TTS directory size: {format_size(total_spark_size)}")
    ...
    try:
        ...
        if os.path.exists(RUNPOD_VOLUME):
            ...
            if free_bytes < 2 * 1024**3:
                logger.warning(f"Low disk space! Only {format_size(free_bytes)} available")
