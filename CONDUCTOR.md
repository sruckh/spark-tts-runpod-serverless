...
- **Tech Stack**: Python 3.12+, PyTorch 2.5+, Transformers (Qwen2.5), Gradio, NumPy, SoundFile
- **Main File**: cli/SparkTTS.py (6,475 total Python lines across 29 files)
- **Core Mechanic**: LLM-based text-to-speech with single-stream decoupled speech tokens
- **Key Integration**: HuggingFace models, BiCodec audio tokenizer, Triton inference server
- **Platform Support**: Linux (primary), Windows (community), CUDA/MPS/CPU inference
- **DO NOT**: Use for unauthorized voice cloning, impersonation, or malicious deepfakes
...
## Quick Reference
**SparkTTS Class**: `cli/SparkTTS.py:27-50+` - Main inference engine with model loading
**Audio Tokenizer**: `sparktts/models/audio_tokenizer.py` - BiCodec tokenization pipeline
**Web Interface**: `webui.py:29-48` - Gradio UI with device detection
**CLI Inference**: `cli/inference.py` - Command-line synthesis interface
**Token Processing**: `sparktts/utils/token_parser.py` - Speech token utilities and mappings
**Config Loading**: `sparktts/utils/file.py` - YAML configuration management
**Wave Generation**: `sparktts/modules/encoder_decoder/wave_generator.py` - Audio synthesis core
**Speaker Encoding**: `sparktts/modules/speaker/speaker_encoder.py` - Voice representation
**Finite Scalar Quantization**: `sparktts/modules/fsq/finite_scalar_quantization.py` - Token quantization
**Triton Deployment**: `runtime/triton_trtllm/model_repo/` - Production serving configuration

## Current State
- [x] Core inference pipeline (SparkTTS class)
- [x] Zero-shot voice cloning capability
- [x] Bilingual support (Chinese/English)
- [x] Cross-lingual and code-switching
- [x] Controllable speech generation (gender, pitch, rate)
- [x] Web UI with Gradio interface
- [x] CLI tools and example scripts
- [x] Triton TensorRT-LLM runtime support
- [x] HuggingFace model distribution
- [ ] Training code release (planned)
- [ ] VoxBox dataset release (planned)

## Development Workflow
1. **Environment Setup**: `conda create -n sparktts python=3.12 && pip install -r requirements.txt`
2. **Model Download**: `git clone https://huggingface.co/SparkAudio/Spark-TTS-0.5B pretrained_models/Spark-TTS-0.5B`
3. **Basic Testing**: `cd example && bash infer.sh` (uses prompt_audio.wav)
4. **Web UI Development**: `python webui.py --device 0` for interactive testing
5. **Production Deployment**: Configure Triton runtime in `runtime/triton_trtllm/`
6. **Quality Validation**: Test with demos in `src/demos/` directory
...
❌ **Don't bypass model config validation** - SparkTTS requires specific config.yaml format
❌ **Don't modify model weights without understanding** - Use proper fine-tuning procedures
❌ **Don't ignore device compatibility checks** - webui.py handles CUDA/MPS/CPU detection
❌ **Don't use raw audio input** - Must go through BiCodec tokenizer pipeline
❌ **Don't violate ethical usage guidelines** - No unauthorized voice cloning or impersonation
❌ **Don't skip audio preprocessing** - Use sparktts.utils.audio utilities for proper format
